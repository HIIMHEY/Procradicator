...
export function FriendSearch({ userId }: FriendSearchProps) {
  ...
  return (
    ...
    >
      <View className="min-h-80 rounded-3xl bg-[#EAF0FF] p-5">
        <Input size="xl" className="rounded-2xl border-0 bg-white">
          <InputSlot className="pl-4">
            <InputIcon as={Search} className="text-slate-500" />
          </InputSlot>
          <InputField
        ...
        {!username.trim() ? (
          <View className="flex-1 items-center justify-center px-4 py-10">
            ...
            <Text className="mt-5 text-center text-xl font-medium text-slate-900">
            ...
            {(search.data ?? []).map((user) => {
              ...
              return (
                <View
                  ...
                  className="min-h-20 flex-row items-center rounded-xl bg-white px-4 py-3"
                >
                  <Text className="flex-1 text-base text-slate-900">{user.username}</Text>
                  {isFriend || isSent ? (
                    <View className="rounded-xl bg-blue-50 px-4 py-2">
                    ...
                    <Button
                      ...
                      className="h-11 w-12 rounded-xl bg-blue-50 p-0"
                    >
                      <ButtonIcon as={UserPlus} className="text-blue-600" />
                    </Button>
                  )}
                </View>
              );
