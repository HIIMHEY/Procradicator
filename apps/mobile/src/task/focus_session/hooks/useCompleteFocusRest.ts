...
const completeFocusRest = async (sessionId: string): Promise<FocusSession> => {
  const res = await fetch(API_ROUTES.FOCUS_SESSIONS.REST_COMPLETE(sessionId), {
    method: 'POST',
    credentials: 'include',
  });
  if (!res.ok) {
...
export default function useCompleteFocusRest(sessionId: string, taskId: string) {
  const client = useQueryClient();
  return useMutation({
    mutationFn: () => completeFocusRest(sessionId),
    onSettled: () => {
