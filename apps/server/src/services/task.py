...
class TaskService:
    ...
    async def create_map(self, roadmap_data: CreateTask, user_id: UUID) -> Task:
        try:
            return await self.task_repo.create_graph(roadmap_data, user_id)
        except DatabaseError as e:
    ...
    async def read_next_subtask(
        ...
        task: Task = await self.read_map(task_id, user_id)
        active = [s for s in task.subtasks if not s.is_done and s.deleted_at is None]
        current = next((s for s in task.subtasks if s.id == current_subtask_id), None)
        if current is None:
